...
namespace TaskLayer
{
                                       ...
                                       double qValueFilter)
	{
            ...
            //_pepxml->date(tmi->tm_year, tmi->tm_mon, tmi->tm_mday, tmi->tm_hour,
            ...
            //tempVar18->search_database()->type() = pepXML::msms_pipeline_analysisMsms_run_summarySearch_summarySearch_databaseType::AA;
            ...
            tempVar18->enzymatic_search_constraint()->max_num_internal_cleavages() = std::to_string(CommonParameters->getDigestionParams()->getMaxMissedCleavages());
            tempVar18->parameter = para.ToArray();
            tempVar16->search_summary = {tempVar18};
            _pepxml->msms_run_summary = {tempVar16};
            
            _pepxml->msms_run_summary[0]->spectrum_query = std::vector<pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_query*>(psms.size());
            
            auto searchHits = std::vector<pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_querySearch_resultSearch_hit*>();
            
            for (auto psm : psms)
            {
                PeptideWithSetModifications *peptide = psm->BestMatchingPeptides.First().Peptide;
                
                auto mods = std::vector<pepXML::modInfoDataTypeMod_aminoacid_mass*>();
                for (auto mod : peptide->AllModsOneIsNterminus)
                {
                    auto pepXmlMod = new pepXML::modInfoDataTypeMod_aminoacid_mass();
                    ...
                    //C# TO C++ CONVERTER TODO TASK: A 'delete pepXmlMod' statement was not added since pepXmlMod was
                ...
                auto searchHit = new pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_querySearch_resultSearch_hit();
                ...
                searchHit->calc_neutral_pep_mass = static_cast<float>((psm->getPeptideMonisotopicMass().has_value()) ? psm->getPeptideMonisotopicMass().value() : NAN);
                searchHit->massdiff = ((psm->getPeptideMonisotopicMass().has_value()) ? std::to_string(psm->getScanPrecursorMass() - psm->getPeptideMonisotopicMass().value) : "Ambiguous");
            ...
            {
                pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_query *tempVar22 = new pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_query();
                ...
                pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_querySearch_result *tempVar23 = new pepXML::msms_pipeline_analysisMsms_run_summarySpectrum_querySearch_result();
