...
namespace TaskLayer
{
	...
	private:
		...
		std::wstring privateCustomMdac;
		double privateMaxFragmentSize = 0;
		double privateHistogramBinTolInDaltons = 0;
		std::unordered_map<std::wstring, int> privateModsToWriteSelection;
	...
	public:
		...
		std::wstring getCustomMdac() const;
		void setCustomMdac(const std::wstring &value);
		...
		std::unordered_map<std::wstring, int> getModsToWriteSelection() const;
		void setModsToWriteSelection(const std::unordered_map<std::wstring, int> &value);
