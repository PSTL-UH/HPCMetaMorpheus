...
namespace Test
{
	...
	{
		Protease *protease = new Protease(L"Customized Protease", CleavageSpecificity::Full, nullptr, nullptr, std::vector<DigestionMotif*> {new DigestionMotif(L"K", nullptr, 1, L"")});
		...
		Assert::AreEqual(L"QQQ", allPsmsArray[0]->getBaseSequence());
...
//C# TO C++ CONVERTER TODO TASK: A 'delete protease' statement was not added since protease was passed to a method or constructor. Handle memory management manually.
	...
	{
		DigestionParams tempVar(protease: L"Customized Protease", maxMissedCleavages: 0, minPeptideLength: 1);
		CommonParameters *CommonParameters = new CommonParameters(digestionParams: &tempVar, scoreCutoff: 1);
		...
		Assert::AreEqual(L"QXQ", allPsmsArray[0]->getBaseSequence());
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestModernSearchEngine()
	{
		...
		DigestionParams tempVar2(protease: L"Customized Protease", minPeptideLength: 1);
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestModernSearchFragmentEdges()
	{
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"K", nullptr)};

		auto indexEngine = new IndexingEngine(proteinList, std::vector<Modification*>(), std::vector<Modification*>(), 1, DecoyType::Reverse, CommonParameters, SearchParameters->getMaxFragmentSize(), false, std::vector<FileInfo*>(), std::vector<std::wstring>());
		auto indexResults = static_cast<IndexingResults*>(indexEngine->Run());

		TestDataFile tempVar3();
		CommonParameters tempVar4();
		auto listOfSortedms2Scans = MetaMorpheusTask::GetMs2Scans(&tempVar3, L"", &tempVar4).OrderBy([&] (std::any b)
		{
			b::PrecursorMass;
		})->ToArray();
		...
		std::vector<PeptideSpectralMatch*> allPsmsArray(listOfSortedms2Scans.size());
		ModernSearchEngine tempVar5(allPsmsArray, listOfSortedms2Scans, indexResults->getPeptideIndex(), indexResults->getFragmentIndex(), 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), new std::vector<std::wstring>());
		(&tempVar5)->Run();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestNonViablePSM()
	{
		...
		DigestionParams tempVar(protease: L"Customized Protease");
		...
		//NonSpecific
		NonSpecificEnzymeSearchEngine tempVar5(allPsmsArrays, listOfSortedms2Scans, indexResults->getPeptideIndex(), indexResults->getFragmentIndex(), indexResults->getFragmentIndex(), 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), new std::vector<std::wstring>());
		(&tempVar5)->Run();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestModernSearchEngineWithWeirdPeptide()
	{
		...
		DigestionParams tempVar(protease: L"Customized Protease", minPeptideLength: 1);
		CommonParameters *CommonParameters = new CommonParameters(digestionParams: &tempVar, scoreCutoff: 1);
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"MNNNKQXQ", nullptr)};
		...
		std::vector<PeptideSpectralMatch*> allPsmsArray(listOfSortedms2Scans.size());
		auto engine = new ModernSearchEngine(allPsmsArray, listOfSortedms2Scans, indexResults->getPeptideIndex(), indexResults->getFragmentIndex(), 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), std::vector<std::wstring>());
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestNonSpecificEnzymeSearchEngineSingleN()
	{
		...
		{
			new DigestionMotif(L"K", nullptr, 0, nullptr),
			new DigestionMotif(L"K", nullptr, 1, nullptr),
			new DigestionMotif(L"G", nullptr, 0, nullptr),
			new DigestionMotif(L"G", nullptr, 1, nullptr)
		};
		Protease *protease = new Protease(L"single N", CleavageSpecificity::SingleN, nullptr, nullptr, motifs);
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		...
		PpmTolerance tempVar(5);
		CommonParameters *CommonParameters = new CommonParameters(dissociationType: DissociationType::HCD, precursorMassTolerance: &tempVar, digestionParams: dp, scoreCutoff: 1, addCompIons: true);

		auto myMsDataFile = new TestDataFile(L"Yes, I'd like one slightly larger please");
		auto variableModifications = std::vector<Modification*>();
		auto fixedModifications = std::vector<Modification*>();

		auto proteinList = std::vector<Protein*> {new Protein(L"GGGGGMNNNKQQQGGGGG", L"TestProtein")};

		auto indexEngine = new IndexingEngine(proteinList, variableModifications, fixedModifications, 1, DecoyType::Reverse, CommonParameters, 100000, false, std::vector<FileInfo*>(), std::vector<std::wstring>());
		auto indexResults = static_cast<IndexingResults*>(indexEngine->Run());
		...
		NonSpecificEnzymeSearchEngine tempVar3(allPsmsArrays, listOfSortedms2Scans, peptideIndex, fragmentIndexDict, fragmentIndexDict, 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), new std::vector<std::wstring>());
		(&tempVar3)->Run();
		...
		Assert::AreEqual(L"QQQGGGG", allPsmsArray[0]->getBaseSequence());
...
//C# TO C++ CONVERTER TODO TASK: A 'delete protease' statement was not added since protease was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestNonSpecificEnzymeSearchEngineSingleCModifications()
	{
		...
		DigestionParams *dp = new DigestionParams(L"singleC", minPeptideLength: 1, fragmentationTerminus: FragmentationTerminus::C, searchModeType: CleavageSpecificity::None);

		PpmTolerance tempVar(5);
		CommonParameters *CommonParameters = new CommonParameters(dissociationType: DissociationType::HCD, digestionParams: dp, scoreCutoff: 5, precursorMassTolerance: &tempVar, addCompIons: true);

		PeptideWithSetModifications *guiltyPwsm = new PeptideWithSetModifications(L"DQPKLLGIETPLPKKE", nullptr);
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"GGGGGCDQPKLLGIETPLPKKEGGGGG", nullptr)};

		auto indexEngine = new IndexingEngine(proteinList, variableModifications, fixedModifications, 1, DecoyType::None, CommonParameters, SearchParameters->getMaxFragmentSize(),true, std::vector<FileInfo*>(), std::vector<std::wstring>());
		...
		allPsmsArrays[2] = std::vector<PeptideSpectralMatch*>(listOfSortedms2Scans.size());
		std::vector<PeptideSpectralMatch*> allPsmsArray = allPsmsArrays[2];
		auto engine = new NonSpecificEnzymeSearchEngine(allPsmsArrays, listOfSortedms2Scans, peptideIndex, fragmentIndexDict, precursorIndexDict, 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), std::vector<std::wstring>());
		auto searchResults = engine->Run();
		...
		proteinList = {new Protein(L"CDQPKLLGIETPLPKKEGGGGG", nullptr)};
		guiltyPwsm = new PeptideWithSetModifications(L"C[Common Fixed:Carbamidomethyl on C]DQPKLLGIETPLPKKE", std::unordered_map<std::wstring, Modification*>
		{
			{L"Carbamidomethyl on C", mod2}
		});
		...
		indexEngine = new IndexingEngine(proteinList, variableModifications, fixedModifications, 1, DecoyType::None, CommonParameters, SearchParameters->getMaxFragmentSize(), true, std::vector<FileInfo*>(), std::vector<std::wstring>());
		...
		CommonParameters tempVar3();
		listOfSortedms2Scans = MetaMorpheusTask::GetMs2Scans(myMsDataFile, L"", &tempVar3).OrderBy([&] (std::any b)
		{
			b::PrecursorMass;
		})->ToArray();
		...
		engine = new NonSpecificEnzymeSearchEngine(allPsmsArrays, listOfSortedms2Scans, peptideIndex, fragmentIndexDict, precursorIndexDict, 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), std::vector<std::wstring>());
		searchResults = engine->Run();
		allPsmsArray[0]->ResolveAllAmbiguities();
		//Check that there is a modification hanging out on the protein n-terminus
		Assert::AreEqual(allPsmsArray[0]->getFullSequence(), guiltyPwsm->FullSequence);

		proteinList = {new Protein(L"GGGGGCDQPKLLGIETPLPKKEGG", nullptr)};
		indexEngine = new IndexingEngine(proteinList, variableModifications, fixedModifications, 1, DecoyType::None, CommonParameters, SearchParameters->getMaxFragmentSize(), true, std::vector<FileInfo*>(), std::vector<std::wstring>());
		indexResults = static_cast<IndexingResults*>(indexEngine->Run());
		peptideIndex = indexResults->getPeptideIndex();
		fragmentIndexDict = indexResults->getFragmentIndex();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestNonSpecificEnzymeSearchEngineSingleNModifications()
	{
		...
		SearchParameters->setMassDiffAcceptorType(MassDiffAcceptorType::Exact);
		SearchParameters->setLocalFdrCategories(std::vector<FdrCategory> {FdrCategory::NonSpecific});
		DigestionParams *dp = new DigestionParams(L"singleN", minPeptideLength: 1, fragmentationTerminus: FragmentationTerminus::N, searchModeType: CleavageSpecificity::None);
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"GGDQPKLLGIETPLPKKECGGGGG", nullptr)};
		...
		proteinList = {new Protein(L"GGGGGDQPKLLGIETPLPKKEC", nullptr)};
		guiltyPwsm = new PeptideWithSetModifications(L"GGDQPKLLGIETPLPKKEC[Common Fixed:Carbamidomethyl on C]", std::unordered_map<std::wstring, Modification*>
		{
			{L"Carbamidomethyl on C", mod2}
		});
		fragments = guiltyPwsm->Fragment(CommonParameters->getDissociationType(), FragmentationTerminus::Both);
		...
		proteinList = {new Protein(L"GGDQPKLLGIETPLPKKECGGGGG", nullptr)};
		indexEngine = new IndexingEngine(proteinList, variableModifications, fixedModifications, 1, DecoyType::None, CommonParameters, SearchParameters->getMaxFragmentSize(), true, std::vector<FileInfo*>(), std::vector<std::wstring>());
		indexResults = static_cast<IndexingResults*>(indexEngine->Run());
		peptideIndex = indexResults->getPeptideIndex();
		fragmentIndexDict = indexResults->getFragmentIndex();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete CommonParameters' statement was not added since CommonParameters was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestNonSpecificEnzymeSearchEngineSingleC()
	{
		...
		Protease *protease = new Protease(L"single C", CleavageSpecificity::SingleC, nullptr, nullptr, motifs);
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		...
		PpmTolerance tempVar(5);
		CommonParameters *CommonParameters = new CommonParameters(dissociationType: DissociationType::HCD, digestionParams: dp, scoreCutoff: 4, precursorMassTolerance: &tempVar, addCompIons: true);
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"GGGGGMNNNKQQQGGGGG", nullptr)};
		...
		allPsmsArrays[2] = std::vector<PeptideSpectralMatch*>(listOfSortedms2Scans.size());
		std::vector<PeptideSpectralMatch*> allPsmsArray = allPsmsArrays[2];
		auto engine = new NonSpecificEnzymeSearchEngine(allPsmsArrays, listOfSortedms2Scans, peptideIndex, fragmentIndexDict, fragmentIndexDict, 0, CommonParameters, massDiffAcceptor, SearchParameters->getMaximumMassThatFragmentIonScoreIsDoubled(), std::vector<std::wstring>());
		auto searchResults = engine->Run();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete protease' statement was not added since protease was passed to a method or constructor. Handle memory management manually.
	...
	{
		auto protein = new Protein(L"MGGGGGMNNNKQQQMGGGGMGM", L"TestProtein");
		...
		std::vector<DigestionMotif*> motifs =
		{
			...
			new DigestionMotif(L"G", nullptr, 0, nullptr),
			new DigestionMotif(L"G", nullptr, 1, nullptr),
			new DigestionMotif(L"M", nullptr, 0, nullptr),
			new DigestionMotif(L"M", nullptr, 1, nullptr),
			new DigestionMotif(L"N", nullptr, 0, nullptr),
			new DigestionMotif(L"N", nullptr, 1, nullptr),
			new DigestionMotif(L"Q", nullptr, 0, nullptr),
			new DigestionMotif(L"Q", nullptr, 1, nullptr)
		};
		auto protease = new Protease(L"singleN2", CleavageSpecificity::SingleN, nullptr, nullptr, motifs);
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		ModificationMotif motifM;
		ModificationMotif::TryGetMotif(L"M", motifM);
		auto variableModifications = std::vector<Modification*> {new Modification(_originalId: L"16", _target: motifM, _locationRestriction: L"Anywhere.", _monoisotopicMass: 15.994915)};
		...
		auto protein2 = new Protein(std::wstring((std::wstring(L"MGGGGGMNNNKQQQMGGGGMGM")).ToCharArray().Reverse().ToArray()), L"TestProtein");
...
//C# TO C++ CONVERTER TODO TASK: A 'delete protease' statement was not added since protease was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestNonSpecificEnzymeVariableModificationHandlingCTerm()
	{
		...
		auto protease = new Protease(L"singleC2", CleavageSpecificity::SingleC, nullptr, nullptr, motifs);
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		ModificationMotif motifM;
		ModificationMotif::TryGetMotif(L"M", motifM);
		auto variableModifications = std::vector<Modification*> {new Modification(_originalId: L"16", _target: motifM, _locationRestriction: L"Anywhere.", _monoisotopicMass: 15.994915)};
...
//C# TO C++ CONVERTER TODO TASK: A 'delete protease' statement was not added since protease was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestSemiSpecificEnzymeEngineSingleN()
	{
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"GGGGGMNNNKQQQGGGGGGKKRKG", L"TestProtein")};
		...
		auto protease = new Protease(L"SingleN", CleavageSpecificity::None, nullptr, nullptr, motifs);
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		DigestionParams tempVar(protease: protease->Name, minPeptideLength: 5, maxModsForPeptides: 2, fragmentationTerminus: FragmentationTerminus::N, searchModeType: CleavageSpecificity::Semi);
		CommonParameters *CommonParameters = new CommonParameters(dissociationType: DissociationType::HCD, productMassTolerance: productMassTolerance, digestionParams: &tempVar, scoreCutoff: 2, addCompIons: true);

		std::unordered_set<DigestionParams*> digestParams = {CommonParameters->getDigestionParams()};
		auto indexEngine = new IndexingEngine(proteinList, variableModifications, fixedModifications, 1, DecoyType::Reverse, CommonParameters, 100000, true, std::vector<FileInfo*>(), std::vector<std::wstring>());
		auto indexResults = static_cast<IndexingResults*>(indexEngine->Run());
		auto peptideIndex = indexResults->getPeptideIndex();
		auto fragmentIndexDict = indexResults->getFragmentIndex();
		auto precursorIndexDict = indexResults->getPrecursorIndex();
		...
		auto engine = new NonSpecificEnzymeSearchEngine(allPsmsArrays, listOfSortedms2Scans, peptideIndex, fragmentIndexDict, precursorIndexDict, 1, CommonParameters, searchModes, 0, std::vector<std::wstring>());
		auto searchResults = engine->Run();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete myMsDataFile' statement was not added since myMsDataFile was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestSemiSpecificEnzymeEngineSingleC()
	{
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"GGGGGMKNNNQQQGGGGKGG", nullptr, nullptr, nullptr, nullptr, std::vector<ProteolysisProduct*> {new ProteolysisProduct(nullptr, nullptr, L"test")})};

		auto productMassTolerance = new AbsoluteTolerance(0.01);
		auto searchModes = new SinglePpmAroundZeroSearchMode(5);
		Protease *protease = new Protease(L"SingleC", CleavageSpecificity::Full, nullptr, nullptr, std::vector<DigestionMotif*>());
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		DigestionParams tempVar(protease: protease->Name, maxMissedCleavages: 5, minPeptideLength: 5, searchModeType: CleavageSpecificity::None, fragmentationTerminus: FragmentationTerminus::C);
		CommonParameters *CommonParameters = new CommonParameters(scoreCutoff: 1, productMassTolerance: productMassTolerance, digestionParams: &tempVar, dissociationType: DissociationType::HCD, addCompIons: true);
		...
		allPsmsArrays[2] = std::vector<PeptideSpectralMatch*>(listOfSortedms2Scans.size());
		std::vector<PeptideSpectralMatch*> allPsmsArray = allPsmsArrays[2];
		auto engine = new NonSpecificEnzymeSearchEngine(allPsmsArrays, listOfSortedms2Scans, peptideIndex, fragmentIndexDict, fragmentIndexDict, 1, CommonParameters, searchModes, 0, std::vector<std::wstring>());
		auto searchResults = engine->Run();
...
//C# TO C++ CONVERTER TODO TASK: A 'delete myMsDataFile' statement was not added since myMsDataFile was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestClassicSemiProtease()
	{
		...
		auto proteinList = std::vector<Protein*> {new Protein(L"MGGGGGMKNNNQQQGGGGKGKKNKKGN", L"hello")};

		auto productMassTolerance = new AbsoluteTolerance(0.01);
		auto searchModes = new SinglePpmAroundZeroSearchMode(5);

		std::vector<DigestionMotif*> motifs1 = {new DigestionMotif(L"G", nullptr, 1, nullptr)};
		std::vector<DigestionMotif*> motifs2 = {new DigestionMotif(L"N", nullptr, 1, nullptr)};

		auto protease = new Protease(L"semi-trypsin1", CleavageSpecificity::Semi, nullptr, nullptr, motifs1);
		auto protease2 = new Protease(L"semi-trypsin2", CleavageSpecificity::Semi, nullptr, nullptr, motifs2);
		...
		auto listOfSortedms2Scans2 = MetaMorpheusTask::GetMs2Scans(myMsDataFile, L"", &tempVar5).OrderBy([&] (std::any b)
		{
			b::PrecursorMass;
		})->ToArray();

		std::vector<PeptideSpectralMatch*> allPsmsArray2(listOfSortedms2Scans.size());
		ClassicSearchEngine tempVar6(allPsmsArray2, listOfSortedms2Scans2, variableModifications, fixedModifications, proteinList, searchModes, CommonParameters2, new std::vector<std::wstring>());
		(&tempVar6)->Run();
		// Single search mode
		...
		Assert::AreEqual(L"QQQGGGG", allPsmsArray2[0]->getBaseSequence());
...
//C# TO C++ CONVERTER TODO TASK: A 'delete myMsDataFile' statement was not added since myMsDataFile was passed to a method or constructor. Handle memory management manually.
	...
	void SearchEngineTests::TestClassicSemiProteolysis()
	{
		...
		std::vector<ProteolysisProduct*> protprod = {new ProteolysisProduct(9, 21, L"chain")};
		auto proteinList = std::vector<Protein*> {new Protein(L"MGGGGGMKNNNQQQGGGGKLKGKKNKKGN", L"hello", nullptr, nullptr, nullptr, protprod)};

		std::vector<DigestionMotif*> motifs1 = {new DigestionMotif(L"G", nullptr, 1, nullptr)};
		auto protease = new Protease(L"semi-Trypsin", CleavageSpecificity::Semi, nullptr, nullptr, motifs1);
		ProteaseDictionary::Dictionary->Add(protease->Name, protease);
		...
		std::unordered_map<std::wstring, bool> found =
		{
			{L"NNNQQQ", false},
			{L"NNNQQ", false},
			{L"NNNQ", false},
		...
		{
			bool b;
			std::unordered_map<std::wstring, bool>::const_iterator found_iterator = found.find(PWSM.BaseSequence);
...
//C# TO C++ CONVERTER TODO TASK: A 'delete digestParams' statement was not added since digestParams was passed to a method or constructor. Handle memory management manually.
//C# TO C++ CONVERTER TODO TASK: A 'delete protease' statement was not added since protease was passed to a method or constructor. Handle memory management manually.
	}
	...
	void SearchEngineTests::TestClassicSearchOneNterminalModifiedPeptideOneScan()
	{
		...
		Modification *mod = new Modification(_originalId: L"acetylation", _modificationType: L"testModType", _target: motif, _chemicalFormula: ChemicalFormula::ParseFormula(L"C2H2O1"), _locationRestriction: L"Anywhere.");

		//Create the protein and add the mod
		std::vector<Modification*> modlist = {mod};
		int modPositionInProtein = 1;
		auto proteinListForSearch = new Protein(L"AAAGSAAVSGAGTPVAGPTGR", nullptr, oneBasedModifications: std::unordered_map<int, std::vector<Modification*>>
		{
		...
		msDataScanArray[0] = new MsDataScan(massSpectrum: &tempVar, oneBasedScanNumber: 1, msnOrder: 1, isCentroid: true, polarity: Polarity::Positive, retentionTime: 10.0, scanWindowRange: &tempVar2, scanFilter: L"f", mzAnalyzer: MZAnalyzerType::Orbitrap, totalIonCurrent: intensities.Sum(), injectionTime: 1.0, noiseData: nullptr, nativeId: L"scan=" + std::to_wstring(1));

		std::vector<Ms2ScanWithSpecificMass*> scans(1);
		double precursorMz = 884.4541;
		int precursorCharge = 2;
		std::wstring filePath = L"path";
		CommonParameters tempVar3();
		...
		DigestionParams *testDigestionParams = new DigestionParams(protease: L"trypsin", maxMissedCleavages: 0, minPeptideLength: 1, initiatorMethionineBehavior: InitiatorMethionineBehavior::Retain);
		...
		CommonParameters *commonParameters = new CommonParameters(, d, , , , , , , , , , , , , , , productTolerance, precursorTolerance, , , testDigestionParams);

		//Search the scan against the protein
		std::vector<PeptideSpectralMatch*> allPsmsArray(1);
		ClassicSearchEngine tempVar4(allPsmsArray, scans, new std::vector<Modification*>(), new std::vector<Modification*>(), proteins, new SinglePpmAroundZeroSearchMode(20), new CommonParameters(, DissociationType::CID), new std::vector<std::wstring>());
