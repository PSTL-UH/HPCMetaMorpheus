...
//							var peptideInProteinInfo = new List<std::tuple<Protein, DigestionParams, int, int, int, int>>();
//							foreach (var peptide in peptidesWithNotchInfo)
//							{
//								peptideInProteinInfo.Add(new std::tuple<Protein, DigestionParams, int, int, int, int>(peptide.Peptide.Protein, peptide.Peptide.DigestionParams, peptide.Peptide.OneBasedStartResidueInProtein, peptide.Peptide.OneBasedEndResidueInProtein, peptide.Peptide.MissedCleavages, peptide.Notch));
//							}
//
...
//									var pep = new PeptideWithSetModifications(proteinInfo.Item1, proteinInfo.Item2, proteinInfo.Item3, proteinInfo.Item4, originalPep.CleavageSpecificityForFdrCategory, originalPep.PeptideDescription, proteinInfo.Item5, originalPep.AllModsOneIsNterminus, originalPep.NumFixedMods);
...
//					{
//						Parallel.ForEach(Partitioner.Create(0, list.Count), new ParallelOptions { MaxDegreeOfParallelism = commonParameters.MaxThreadsToUsePerFile }, (range, loopState) =>
//							{
