	...
	namespace ModernSearch
	{
		...
		MetaMorpheusEngineResults *ModernSearchEngine::RunSpecific()
		{
			...
			Parallel::ForEach(Partitioner::Create(0, ListOfSortedMs2Scans.size()), tempVar2, [&] (range, loopState)
			{
				...
				for (int i = range::Item1; i < range::Item2; i++)
				{
					...
					// first-pass scoring
					IndexedScoring(allBinsToSearch, scoringTable, byteScoreCutoff, idsOfPeptidesPossiblyObserved, scan->getPrecursorMass(), lowestMassPeptideToLookFor, highestMassPeptideToLookFor, PeptideIndex, MassDiffAcceptor, MaxMassThatFragmentIonScoreIsDoubled);
					...
					for (auto id : idsOfPeptidesPossiblyObserved)
					{
						...
						std::vector<Product*> peptideTheorProducts = peptide->Fragment(commonParameters->getDissociationType(), FragmentationTerminus::Both).ToList();
						...
						bool scoreImprovement = PeptideSpectralMatches[i] == nullptr || (thisScore - PeptideSpectralMatches[i]->getRunnerUpScore()) > -PeptideSpectralMatch::ToleranceForScoreDifferentiation;
            
						if (meetsScoreCutoff && scoreImprovement || commonParameters->getCalculateEValue())
						{
							if (PeptideSpectralMatches[i] == nullptr)
							{
								PeptideSpectralMatches[i] = new PeptideSpectralMatch(peptide, notch, thisScore, i, scan, commonParameters->getDigestionParams(), matchedIons);
							}
							else
							{
								PeptideSpectralMatches[i]->AddOrReplace(peptide, thisScore, notch, commonParameters->getReportAllAmbiguity(), matchedIons);
							}
		...
		std::vector<int> ModernSearchEngine::GetBinsToSearch(Ms2ScanWithSpecificMass *scan)
		{
			...
			for (auto envelope : scan->getExperimentalFragments())
			{
				...
				if (obsFragmentCeilingMass >= FragmentIndex.size())
				{
					obsFragmentCeilingMass = FragmentIndex.size() - 1;

					if (obsFragmentFloorMass >= FragmentIndex.size())
					{
				...
				if (commonParameters->getAddCompIons())
				{
					...
					std::unordered_map<DissociationType*, double>::const_iterator complementaryIonConversionDictionary_iterator = complementaryIonConversionDictionary.find(commonParameters.DissociationType);
					if (complementaryIonConversionDictionary_iterator != complementaryIonConversionDictionary.end()) //TODO: this is broken for EThcD because that method needs two conversions
					{
						...
						if (compFragmentCeilingMass >= FragmentIndex.size())
						{
							compFragmentCeilingMass = FragmentIndex.size() - 1;

							if (compFragmentFloorMass >= FragmentIndex.size())
							{
		...
		void ModernSearchEngine::IndexedScoring(std::vector<int> &binsToSearch, std::vector<unsigned char> &scoringTable, unsigned char byteScoreCutoff, std::vector<int> &idsOfPeptidesPossiblyObserved, double scanPrecursorMass, double lowestMassPeptideToLookFor, double highestMassPeptideToLookFor, std::vector<PeptideWithSetModifications*> &peptideIndex, MassDiffAcceptor *massDiffAcceptor, double maxMassThatFragmentIonScoreIsDoubled)
		{
			...
			for (int i = 0; i < binsToSearch.size(); i++)
			{
				...
				int highestPeptideMassIndex = peptideIdsInThisBin.size() - 1;
				...
				if (!std::isinf(highestMassPeptideToLookFor))
				{
					...
					for (int j = highestPeptideMassIndex; j < peptideIdsInThisBin.size(); j++)
					{
