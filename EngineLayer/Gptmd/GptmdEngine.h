...
namespace EngineLayer
{
	namespace Gptmd
	{
		...
		private:
			...
			const std::unordered_map<std::wstring, Tolerance*> FilePathToPrecursorMassTolerance; // this exists because of file-specific tolerances

		public:
			GptmdEngine(std::vector<PeptideSpectralMatch*> &allIdentifications, std::vector<Modification*> &gptmdModifications, std::vector<std::tuple<double, double>> &combos, std::unordered_map<std::wstring, Tolerance*> &filePathToPrecursorMassTolerance, CommonParameters *commonParameters, std::vector<std::wstring> &nestedIds);
		...
		private:
			static void AddIndexedMod(std::unordered_map<std::wstring, std::unordered_set<std::tuple<int, Modification*>>> &modDict, const std::wstring &proteinAccession, std::tuple<int, Modification*> &indexedMod);
